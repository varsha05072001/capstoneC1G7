...
export class ProductService {
    ...
    private productAscUrl = `${apiUrl}/productbyasc`;
    private productDescUrl = `${apiUrl}/productbydesc`;
    ...
    getAllInPageAsc(page: number, size: number): Observable<any> {
        const url = `${this.productAscUrl}?page=${page}&size=${size}`;
        return this.http.get(url)
            .pipe(
                // tap(_ => console.log(_)),
            )
    }
    getAllInPageDesc(page: number, size: number): Observable<any> {
        const url = `${this.productDescUrl}?page=${page}&size=${size}`;
        return this.http.get(url)
            .pipe(
                // tap(_ => console.log(_)),
            )
    ...
    upload(file):any {
        const UPLOAD_URL ="http://localhost:8080/c1g7/csv/upload";
        ...
        xhr.open('POST', 'http://localhost:8080/c1g7/csv/upload', true);
        xhr.send(formData);
        return xhr;
